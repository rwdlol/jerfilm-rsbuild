...
const TMDB_API_KEY = process.env.TMDB_API_KEY || "9b121a7c344eeb23baad7647d6b2eabe";
...
app.get("/sitemap.xml", async (req, res) => {
  ...
  try {
    ...
    const actorPromise = fetch(`${TMDB_BASE_URL}/person/popular?api_key=${TMDB_API_KEY}&page=1`)
      .then(r => r.json())
      .then(d => d.results || [])
      .catch(() => []);
...
app.get("/api/health", (_req, res) => {
  res.json({ status: "ok", app: "CineStream TMDB Client", sitemap: "/sitemap.xml" });
});
