...
export function filterAdultItems<T extends { adult?: boolean }>(
	items: T[],
): T[] {
...
export async function getPopularMovies(
	...
	const data = await fetchTMDB<{ results: MediaItem[]; total_pages: number }>(
		"movie/popular",
		{ page },
	);
	return { ...data, results: filterAdultItems(data.results) };
...
export async function getPopularPersons(
	page = 1,
): Promise<{ results: Person[]; total_pages: number }> {
	return fetchTMDB<{ results: Person[]; total_pages: number }>(
		"person/popular",
		{ page },
	);
}
