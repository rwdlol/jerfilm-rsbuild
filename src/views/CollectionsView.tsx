...
import { Layers, Film, Sparkles, Play, Star, ChevronLeft, Search } from 'lucide-react';
import { ActiveRoute, MediaItem } from '../types';
import { getImageUrl, searchMulti, discoverMedia } from '../services/tmdb';
...
const FEATURED_COLLECTIONS: CollectionFranchise[] = [
  {
    ...
    description: 'تەواوی فیلمەکانی مارڤڵ لە ئەڤێنجەرز، ئایرۆن مان، تاوەکو سپایدەرمان و ثۆر.',
  ...
  },
  {
    ...
    description: 'لە دارک نایتەکەی کریستۆفەر نۆلانەوە تاوەکو باتمانەکەی ڕۆبێرت پاتینسۆن.',
  ...
  },
  {
    ...
    description: 'کۆکراوەی تەواوی فیلمەکانی جادووگەری هاری پۆتەر و فانتاسیک بیستس.',
  ...
  },
  {
    ...
    description: 'تەواوی بەشەکانی خێرا و تووڕە لەگەڵ دۆمینیک تۆریتۆ و تیمی پێشبڕکێکاران.',
  ...
  },
  {
    ...
    description: 'بەشەکانی جۆن ویک لەگەڵ کیانو ڕیڤس لە جیهانی بکوژە پیشەگەرەکاندا.',
  ...
  },
  {
    ...
    description: 'گەشتە ئەفساناوییەکەی خاکی ناوەڕاست بۆ لەناوبردنی ئەڵقەی دەسەڵات.',
...
export const CollectionsView: React.FC<CollectionsViewProps> = ({ onRouteChange }) => {
  const [activeCollection, setActiveCollection] = useState<CollectionFranchise | null>(FEATURED_COLLECTIONS[0]);
  ...
  const fetchCollectionItems = async (collection: CollectionFranchise) => {
    ...
    try {
      ...
      const res = await discoverMedia('movie', { page: 1, sortBy: 'popularity.desc' });
      if (res && res.results) {
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto animate-in fade-in duration-300">
      <SEO
        ...
        keywords={["کۆکراوەکان", "زنجیرە فیلم", "مارڤڵ کوردی", "هاری پۆتەر کوردی", "Jerfilm"]}
      />
      {/* Title Header */}
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between w-full z-10">
                <Layers className={`w-4 h-4 ${isSelected ? 'text-zinc-950' : 'text-amber-400'}`} />
                <span className={`text-[10px] px-1.5 py-0.5 rounded ${isSelected ? 'bg-zinc-950/20 text-zinc-950' : 'bg-zinc-800 text-zinc-400'}`}>
                  {c.itemCount} بەش
                </span>
              </div>
              <div className="z-10">
          ...
          <div className="mt-8">
            ...
            {loading ? (
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4">
                {[1, 2, 3, 4, 5, 6].map((n) => (
                  <div key={n} className="aspect-[2/3] bg-zinc-800 animate-pulse rounded-2xl" />
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {collectionMovies.map((item) => (
                  <div
                    key={item.id}
                    onClick={() => onRouteChange({ mode: 'movie-detail', id: item.id })}
