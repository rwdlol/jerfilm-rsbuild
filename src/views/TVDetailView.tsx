...
import { TVDetails, Season, Cast, MediaItem, ActiveRoute } from '../types';
import { 
  getTVDetails, 
  getTVSeasonDetails, 
  getCredits, 
...
export const TVDetailView: React.FC<TVDetailViewProps> = ({
  tvId,
  onRouteChange,
  isInWatchlist,
  onToggleWatchlist,
  ...
  if (loading) {
    return (
      <div className="min-h-screen pt-20 flex flex-col items-center justify-center gap-3">
        ...
        <span className="text-xs font-semibold text-zinc-400">تکایە چاوەڕێ بکە... گواستنەوەی زانیارییەکانی زنجیرەکە</span>
      </div>
    );
  }
  ...
  const backdropUrl = getImageUrl(show.backdrop_path || show.poster_path, 'original');
  ...
  return (
    <div className="pb-24">
      <SEO
        ...
        description={show.overview ? tToSorani(show.overview) : `سەیرکردنی زنجیرەی ${show.name} بە ژێرنووسی کوردی بە هەموو وەرزەکان و ئەڵقەکان.`}
        image={backdropUrl || posterUrl}
        type="video.tv_show"
        keywords={[show.name, `زنجیرەی ${show.name}`, "ژێرنووسی کوردی", "زنجیرەی کوردی", "Jerfilm", "سەیرکردنی زنجیرە"]}
        schemaData={{
          "@context": "https://schema.org",
          "@type": "TVSeries",
          "name": show.name,
          "alternateName": show.original_name,
          "description": show.overview ? tToSorani(show.overview) : `زنجیرەی ${show.name} بە ژێرنووسی کوردی`,
          "image": posterUrl,
      ...
      <div className="fixed top-16 right-4 z-30">
        <button
          onClick={() => window.history.length > 1 ? window.history.back() : onRouteChange({ mode: 'tv' })}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 -mt-32 relative z-10">
        <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-start">
          ...
          <div className="w-40 sm:w-52 md:w-64 flex-shrink-0 rounded-2xl overflow-hidden border-2 border-zinc-800 shadow-2xl bg-zinc-900 mx-auto md:mx-0">
            <img src={posterUrl} alt={show.name} className="w-full h-auto object-cover" />
          </div>
          ...
          <div className="flex-1 space-y-4 text-center md:text-right">
            <div>
              <div className="flex items-center justify-center md:justify-start gap-2 flex-wrap mb-2">
                ...
                <span className="text-xs text-zinc-300 font-medium bg-zinc-900/80 px-2.5 py-0.5 rounded-md border border-zinc-800">
                  {show.number_of_seasons} وەرز ({show.number_of_episodes} ئەڵقە)
                </span>
              </div>
            ...
            <div className="flex items-center justify-center md:justify-start gap-3 pt-2 flex-wrap">
              ...
              >
                ...
                <span>
                  {savedItem ? `بەردەوامبوون لە وەرزی ${resumeSeason} ئەڵقەی ${resumeEpisode}` : 'دەستپێکردنی سەیرکردنی وەرزی ۱ ئەڵقەی ۱'}
                </span>
              </button>
        ...
        <section className="my-10 bg-zinc-900/80 border border-zinc-800 rounded-2xl p-4 sm:p-6 backdrop-blur-md">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 pb-4 border-b border-zinc-800">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-white">ئەڵقەکان و وەرزەکان</h2>
            </div>

            {/* Season Dropdown */}
            <div className="flex items-center gap-2">
              <span className="text-xs text-zinc-400 font-semibold">هەڵبژاردنی وەرز:</span>
              <select
          ...
          {seasonLoading ? (
            ...
            <div className="space-y-3">
              {seasonData.episodes.map((episode) => {
                const stillUrl = getImageUrl(episode.still_path || show.backdrop_path, 'w500');
                const isPlaying = resumeSeason === selectedSeasonNum && resumeEpisode === episode.episode_number;

                return (
                  <div
                    key={episode.id}
                    onClick={() => handlePlayEpisode(selectedSeasonNum, episode.episode_number)}
                  ...
                  >
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                      {/* Thumbnail */}
                      <div className="relative w-28 sm:w-36 aspect-video rounded-lg overflow-hidden flex-shrink-0 bg-zinc-900">
                        <img
                          src={stillUrl}
                          alt={episode.name}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-zinc-400 mt-0.5">
                          ...
                          {episode.vote_average > 0 && (
                            <>
                              <span>•</span>
                              <span className="text-amber-400 font-bold">★ {episode.vote_average.toFixed(1)}</span>
                            </>
                          )}
                        </div>

                        <p className="text-[11px] text-zinc-400 line-clamp-2 mt-1 font-normal">
                          {episode.overview || 'هیچ کورتەیەک بۆ ئەم ئەڵقەیە بەردەست نییە.'}
                        </p>
                      </div>
                    </div>
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handlePlayEpisode(selectedSeasonNum, episode.episode_number);
                      }}
