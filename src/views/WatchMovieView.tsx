...
export default function WatchMovieView({
	movieId,
	onRouteChange,
	isInWatchlist,
	onToggleWatchlist,
}: WatchMovieViewProps) {
	// Coerce movieId to a number
	...
	const releaseYear = movie ? (movie.release_date || "").substring(0, 4) : "";
	...
	useEffect(() => {
		...
		async function fetchMovieData() {
			...
			try {
				...
				// Fetch critical details first with a catch fallback
				const movieData = await getMovieDetails(numericMovieId).catch((err) => {
				...
				// End loading state as soon as the main movie resource has resolved
				setLoading(false);

				if (movieData) {
					// Fetch recommendations in the background
					getRecommendations("movie", numericMovieId)
						.then((recsData) => {
							if (isMounted) {
	...
	const handleShare = async () => {
		const shareTitle = `فیلمی ${movie?.title || ""} (${releaseYear})`;
		const shareText = `سەیرکردنی فیلمی ${movie?.title || ""} بە ژێرنووسی کوردی`;
		...
		return (
			<div className="min-h-screen pt-20 flex flex-col items-center justify-center gap-3">
				...
				<span className="text-xs font-semibold text-zinc-400">
					تکایە چاوەڕێ بکە... ئامادەکردنی پەخشی فیلمەکە
				</span>
			</div>
		);
	}
	...
	const streamUrl = currentServer?.getMovieUrl
		? currentServer.getMovieUrl(numericMovieId)
		: "";
		...
		>
			...
			<div className="pt-16 pb-3 px-4 sm:px-6 max-w-7xl mx-auto flex items-center justify-between">
				<button
					type="button"
					onClick={() =>
						onRouteChange({ mode: "movie-detail", id: numericMovieId })
					}
