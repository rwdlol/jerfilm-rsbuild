...
import { ArrowRight, Star, Film, Tv, Calendar, MapPin, Play, Award, Sparkles } from 'lucide-react';
import { ActiveRoute, PersonDetails, MediaItem } from '../types';
...
export const ActorDetailView: React.FC<ActorDetailViewProps> = ({ actorId, onRouteChange }) => {
    ...
    return (
      <div className="min-h-screen pt-28 pb-20 px-4 sm:px-6 max-w-7xl mx-auto flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs font-bold text-zinc-400">تکایە چاوەڕێ بکە...</span>
        </div>
      </div>
    );
  }
  ...
  if (!details) {
    return (
      <div className="min-h-screen pt-28 pb-20 px-4 sm:px-6 max-w-3xl mx-auto text-center">
        <h2 className="text-xl font-bold text-white mb-4">زانیارییەکانی ئەکتەر نەدۆزرایەوە</h2>
        <button
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto animate-in fade-in duration-300">
      <SEO
        ...
        description={details.biography ? details.biography.slice(0, 160) : `هەموو فیلم و زنجیرەکانی ${details.name} بە ژێرنووسی کوردی.`}
        image={profileUrl}
        type="profile"
        keywords={[details.name, `فیلمەکانی ${details.name}`, "ئەکتەر", "Jerfilm"]}
        schemaData={{
          ...
          "description": details.biography ? details.biography.slice(0, 200) : undefined
        }}
      />
      {/* Back Button */}
      <button
      ...
      <div className="rounded-3xl bg-zinc-900/80 border border-zinc-800 p-6 sm:p-8 mb-12 shadow-2xl flex flex-col md:flex-row gap-8 items-center md:items-start">
        ...
        <div className="flex-1 text-center md:text-right">
          ...
          {details.biography ? (
            ...
            <p className="text-xs text-zinc-500">هیچ کورتەیەکی ژیاننامە بەردەست نییە.</p>
          )}
        </div>
      </div>
