...
import { WatchlistItem, WatchStatus, ActiveRoute } from '../types';
...
interface WatchlistViewProps {
  ...
  onUpdateStatus: (id: number, type: 'movie' | 'tv', status: WatchStatus) => void;
    ...
    <div className="pt-20 pb-24 max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 bg-zinc-900 border border-zinc-800 p-1 rounded-xl">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${
                filterType === 'tv' ? 'bg-amber-400 text-zinc-950' : 'text-zinc-400 hover:text-white'
              }`}
            >
              زنجیرەکان
            </button>
      ...
      {filteredItems.length === 0 ? (
        <div className="py-20 text-center bg-zinc-900/50 border border-zinc-800/80 rounded-2xl p-8 max-w-md mx-auto">
          ...
          <h3 className="text-base font-bold text-white mb-1">لیستی سەیرکردن بەتاڵە</h3>
            ...
            return (
              ...
              >
                ...
                <div
                  onClick={() =>
                    onRouteChange({
                      mode: item.mediaType === 'movie' ? 'movie-detail' : 'tv-detail',
                      id: item.id,
                    })
                  }
                  className="w-24 h-36 flex-shrink-0 rounded-xl overflow-hidden bg-zinc-950 relative cursor-pointer group-hover:scale-105 transition-transform"
                >
                  ...
                  <div>
                    ...
                    {item.mediaType === 'tv' && item.lastWatchedSeason && (
                      <div className="mt-2 text-[10px] font-bold text-amber-400 bg-amber-400/10 border border-amber-400/30 px-2 py-0.5 rounded-md inline-block">
                        پێشکەوتن: وەرزی {item.lastWatchedSeason} ئەڵقەی {item.lastWatchedEpisode || 1}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-3 space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <select
                        value={item.status}
                        onChange={(e) =>
                          onUpdateStatus(item.id, item.mediaType, e.target.value as WatchStatus)
                        }
                    ...
                    >
                      ...
                      <span>{item.mediaType === 'tv' && item.lastWatchedSeason ? 'بەردەوامبوون لە سەیرکردن' : 'سەیرکردنی ئێستا'}</span>
                    </button>
                  </div>
                </div>
              </div>
