...
import { ChevronRight, ChevronLeft, Flame, Star, Play } from 'lucide-react';
...
export const Top10Slider: React.FC<Top10SliderProps> = ({
  ...
  const toKurdishNumber = (num: number) => {
    const kurdishDigits = ['٠', '١', '٢', '٣', '٤', '٥', '٦', '٧', '٨', '٩', '١٠'];
    return kurdishDigits[num] || num.toString();
  };

  return (
    <div className="my-6 relative group/slider">
      {/* Header */}
      <div className="flex items-center justify-between mb-3 px-4 sm:px-6">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-gradient-to-tr from-amber-500 via-amber-300 to-amber-600 p-0.5 shadow-md shadow-amber-500/20">
      ...
      <div
        ref={sliderRef}
        className="flex gap-4 overflow-x-auto no-scrollbar px-4 sm:px-6 py-3 snap-x scroll-smooth"
      >
        {items.slice(0, 9).map((item, index) => {
          ...
          return (
            <div
              key={item.id}
              onClick={handleClick}
              className="flex-none w-[170px] sm:w-[210px] snap-start relative group cursor-pointer select-none"
            >
              <div className="flex items-end">
                {/* Large Stylized Rank Number */}
                <span className="text-[80px] sm:text-[110px] font-black leading-none tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-zinc-100 via-zinc-400 to-zinc-800 opacity-90 select-none pointer-events-none -mr-4 sm:-mr-6 z-10 drop-shadow-[0_10px_20px_rgba(0,0,0,0.8)] font-display">
                ...
                <div className="relative w-full aspect-[2/3] rounded-2xl overflow-hidden bg-zinc-900 border border-zinc-800 group-hover:border-amber-400/80 transition-all duration-300 group-hover:shadow-xl group-hover:shadow-amber-400/20 z-0">
                  ...
                  {/* Top VIP Badge */}
                  <div className="absolute top-2 right-2 bg-gradient-to-r from-amber-400 to-amber-500 text-zinc-950 text-[9px] font-black uppercase px-2 py-0.5 rounded-md shadow-md">
                    TOP {rank}
                  </div>

                  {/* Bottom Info & Play button overlay */}
                  <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-zinc-950 via-zinc-950/90 to-transparent flex flex-col justify-end">
                    <div className="flex items-center gap-1.5 text-[10px] text-amber-400 font-bold mb-1">
                      <Star className="w-3 h-3 fill-amber-400" />
                      <span>{rating}</span>
                      <span className="text-zinc-400">• {type === 'movie' ? 'فیلم' : 'زنجیرە'}</span>
                    </div>
