...
import { Film, Search, Bookmark, Sparkles, X, Tv, Home, Layers, Users, Info } from 'lucide-react';
      ...
      <header
        className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
          isScrolled
            ...
            : 'bg-gradient-to-b from-[#060709]/95 via-[#060709]/60 to-transparent py-3.5'
        }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
          ...
          >
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-amber-500 via-amber-300 to-amber-600 p-0.5 shadow-lg shadow-amber-500/20 group-hover:scale-105 transition-transform">
            ...
            <div className="flex flex-col text-right">
              <span className="text-xl font-black tracking-wider text-white font-display flex items-center gap-1">
                <span>Jerfilm</span>
                <span className="text-amber-400 text-sm font-black bg-amber-400/10 px-1.5 py-0.2 rounded border border-amber-400/30">.VIP</span>
              </span>
              <span className="text-[10px] font-bold tracking-widest text-zinc-400 -mt-0.5">
                www.jerfilm.vip
              </span>
            </div>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-1 bg-zinc-900/80 border border-zinc-800/90 p-1.5 rounded-full text-xs font-semibold backdrop-blur-md shadow-inner">
            ...
            <Link
              to="/movies"
              className={`px-3.5 py-1.5 rounded-full transition-all flex items-center gap-1.5 ${
                pathname.startsWith('/movies') || pathname.startsWith('/movie/') || pathname.startsWith('/watch-movie/')
                  ? 'bg-amber-400 text-zinc-950 font-bold shadow-md shadow-amber-400/20'
                  : 'text-zinc-300 hover:text-white hover:bg-zinc-800/60'
              }`}
            >
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-start justify-center p-4 pt-20 animate-in fade-in duration-200">
          <div className="w-full max-w-2xl bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl p-4 overflow-hidden">
            ...
            <div className="mt-4 pt-3 border-t border-zinc-800/50 flex items-center justify-between text-xs text-zinc-500">
              ...
              <span className="hidden sm:inline">داگرتنی Enter بۆ گەڕانی تەواو</span>
            </div>
          </div>
        </div>
      )}
