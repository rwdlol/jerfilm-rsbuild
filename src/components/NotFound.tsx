import { SearchX } from 'lucide-react';
import { Link } from 'react-router';

export default function NotFound() {
  return (
    <div className="flex flex-col text-center justify-center items-center gap-2 h-fit min-h-[calc(100dvh-64px)]">
      <SearchX size={48} strokeWidth={1.25} className="text-red-600" />
      <h1 className="text-xl text-white font-bold">نەدۆزرایەوە</h1>
      <p className="max-w-md">هیچ پێناسەیەک نەدۆزرایەوە</p>
      <Link
        to="/"
        className="mt-2 bg-gold text-zinc-950 font-bold p-2 px-4 rounded-full"
      >
        بڕۆ بۆ پەڕەی سەرەکی
      </Link>
    </div>
  );
}
