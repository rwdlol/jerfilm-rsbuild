...
const SORT_OPTIONS = [
  ...
  { value: 'popular', label: 'فیلمە بەناوبانگەکان (Popular)' },
  ...
  { value: 'upcoming', label: 'فیلمی داهاتوو (Upcoming)' },
];
...
export default function MoviePage() {
  ...
  // 1. Fetch featured popular movies for the top Carousel
  const [popularData] = useTMDB<TMDBResponseList<Movie>>(
    '/trending/movie/week?page=1',
  );

  // Slice first 5 popular movies for the Carousel
  const featuredSlides: Movie[] = (popularData?.results || [])
    .slice(0, 5)
    .map((movie) => ({
      ...movie,
      cta_text: 'سەیرکردنی فیلمەکە',
      cta_link: `/movie/${movie.id}`,
      cta_type: 'gold',
    }));

  // 2. Fetch the Movie Genre list dynamically
  ...
  let apiPath = '';
  if (searchQuery.trim() !== '') {
    apiPath = `/search/movie?include_adult=false&query=${encodeURIComponent(searchQuery)}&page=${page}`;
    ...
    apiPath = `/discover/movie?include_adult=false&include_video=false&sort_by=popularity.desc&page=${page}${genresParam}${countriesParam}`;
  } else if (filterType === 'trending_day') {
    apiPath = `/trending/movie/day?page=${page}`;
  } else if (filterType === 'trending_week') {
    apiPath = `/trending/movie/week?page=${page}`;
  } else if (filterType === 'popular') {
    apiPath = `/movie/popular?page=${page}`;
  } else if (filterType === 'top_rated') {
    apiPath = `/movie/top_rated?page=${page}`;
  } else if (filterType === 'upcoming') {
    apiPath = `/movie/upcoming?page=${page}`;
  }

  // Fetch filtered/searched movies
  const [moviesData, moviesLoading, moviesError] =
    useTMDB<TMDBResponseList<Movie>>(apiPath);
  ...
  return (
    <div className="flex flex-col w-full gap-8 pb-8">
      ...
      {/* B. Filter and Control Hub (All custom styled dropdowns) */}
          ...
          <div className="flex items-center justify-between border-b border-zinc-900 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-white">
                دۆزینەوە و فلتەرکردنی فیلمەکان
              </h2>
            </div>
          </div>
      ...
      {/* C. Movies Grid Section (Responsive 2 to 5 columns) */}
      <section className="px-4 md:px-8 flex flex-col gap-6">
        {moviesLoading && (
          <div className="text-zinc-500 text-sm text-center py-12">
            بارکردنی فیلمە فلتەرکراوەکان...
          </div>
        )}

        {moviesError && (
          <div className="text-red-500 text-sm text-center py-12">
            هەڵەیەک لە بارکردنی داتاکاندا ڕوویدا: {moviesError.message}
          </div>
        )}

        {!moviesLoading && !moviesError && moviesData?.results && (
          <>
            ...
            {moviesData.total_pages > 1 && (
              <div className="flex items-center justify-center gap-4 border-t border-zinc-900 pt-6 mt-4 select-none">
                ...
                <span className="text-xs text-zinc-400">
                  ...
                  {moviesData.total_pages.toLocaleString('ku-IQ')}
                </span>
